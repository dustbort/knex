import { columnize } from './wrappingFormatter';
import type Raw from "../raw";
import type Client from "../client";

export function replaceRawArrBindings(raw: Raw, client: Client) {
...
export function replaceKeyBindings(raw: Raw, client: Client) {
  const bindingsHolder = {
    bindings: [],
  };
  const builder = raw;
